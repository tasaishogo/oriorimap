...
- **実行モードは波（run）ごとに選択する**（project-autopilot Phase 1で毎回確定）:
  ...
  - **★T005-b の事前確認（2026-08-12・未検証事項あり）**: gate は Processed テンプレート全体を whitelist で評価するため、初回実行の前に **`tools/proj gate oriorimap oriorimap-dev <changeset> --dry-run` で必ず判定を先取りする**（mvm-poc 助言）。判明済みの論点:
    - `10-iam-role.guard` は**全 `AWS::IAM::Role` に `PermissionsBoundary` を要求**する。本テンプレートの IAM ロールは SAM 自動生成のため既定では boundary が付かず、そのままでは弾かれる（かつ CFn サービスロール側の boundary Deny で実デプロイも失敗する）。→ `Globals.Function.PermissionsBoundary` を追加済み。**当初のパラメータ `UseMvmBoundary` + `Fn::If` 方式は、T005-b 初回 dry-run（2026-08-12）で guard に「検証不能（fail-closed）」として blocked された**――gate は Processed テンプレートを静的検証するため、boundary は素の `Fn::Sub` 文字列形（または `Fn::ImportValue`）で直書きする必要がある。同日、パラメータと条件を撤去して直書きへ改めた（mvm 環境以外へ流用する場合は template の該当2箇所を編集する）
